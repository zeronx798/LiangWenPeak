...
新增中国法定节假日全天闲时计费，并完善菜单状态、ETA 显示与 portable 条件单例。

### 亮点

* 中国法定节假日全天按闲时半价计算，调休上班的自然周末仍保持周末闲时规则
* 下一时段与峰谷通知跨过完整假期，仅在节假日造成额外延后时显示明确日期
* 修复“始终置顶”持久化，以及通知菜单图标、文案和勾选状态不同步的问题
* ETA 显示向下舍入到整分钟，不再显示秒，同时保留内部秒级计算精度
* 同一 canonical `data/` 最多运行一个逻辑实例，重复启动会激活已有窗口
* 不同 portable 副本使用不同 `data/` 时仍可独立运行
* 将受管 2026 节假日日历随 portable 应用 payload 发布，且继续排除用户 `data/`

### 下载

`LiangWenPeak-1.1.3-windows-x64.zip`

SHA256：

```text
0FBC151F1241C761A76ADF2A599029F255B42331D8D9FD082BC79CCB23D1DFA7
```
